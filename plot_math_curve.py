...
def plot_curve(
    steps,
    scores,
    save_prefix="math_step_score_curve",
    title="MATH Step Score over Training",
):
    plt.rcParams.update({
        "font.family": "serif",
        "font.size": 14,
...
def main():
    ...
    steps, scores = load_step_scores(eval_dir, benchmark="math")

    if len(steps) == 0:
        raise ValueError(f"No MATH json files found in {eval_dir}")
