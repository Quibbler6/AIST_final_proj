...
def plot_curve(
    steps,
    scores,
    save_prefix="olympiad_bench_step_score_curve",
    title="OlympiadBench Step Score over Training",
):
    plt.rcParams.update({
        "font.family": "serif",
        "font.size": 14,
...
def main():
    ...
    steps, scores = load_step_scores(eval_dir, benchmark="olympiad_bench")

    if len(steps) == 0:
        raise ValueError(f"No OlympiadBench json files found in {eval_dir}")
    ...
    plot_curve(
        steps,
        scores,
        save_prefix="olympiad_bench_step_score_curve_qwen_template",
        title="OlympiadBench Step Score over Training",
    )
